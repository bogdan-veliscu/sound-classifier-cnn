...
def train_and_evaluate(
    ...
    for epoch in range(params.epochs):
        ...
        writer.add_scalar(
            "data{}/trainingLoss{}".format(params.dataset_name, split), avg_loss, epoch
        )
        writer.add_scalar(
    ...
    for i in range(1, params.num_folds + 1):
        if params.dataaug:
            train_loader = dataloaders.datasetaug.fetch_dataloader(
                "{}training128mel{}.pkl".format(params.data_dir, i),
                params.dataset_name,
                params.batch_size,
                params.num_workers,
                "train",
            ...
            val_loader = dataloaders.datasetaug.fetch_dataloader(
                "{}validation128mel{}.pkl".format(params.data_dir, i),
                params.dataset_name,
                params.batch_size,
                params.num_workers,
                "validation",
        ...
        if params.model == "densenet":
            model = models.densenet.DenseNet(params.dataset_name, params.pretrained).to(
                device
            )
        elif params.model == "resnet":
            model = models.resnet.ResNet(params.dataset_name, params.pretrained).to(
                device
            )
        elif params.model == "effnet":
            model = models.effnet.EffNet(params.dataset_name, params.pretrained).to(
                device
            )
        elif params.model == "mobilenet":
            model = models.mobilenet.MobileNet(
                params.dataset_name, params.pretrained
            ).to(device)
